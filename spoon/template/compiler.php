	...
	protected function parseIncludes($content)
	{
		...
		if(preg_match_all($pattern, $content, $matches, PREG_SET_ORDER))
		{
			...
			foreach($matches as $match)
			{
				...
				if($this-if($this->getForceCompile() || !file_exists($this->getCompileDirectory() .\'/\' . $this->getCompileName($include, \'' . dirname(realpath($this->template)) . '\'))) $this->compile(\'' . dirname(realpath($this->template)) . '\', $include);>getForceCompile()) $this->compile(\'' . dirname(realpath($this->template)) . '\', $include);
